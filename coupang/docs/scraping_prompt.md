# Coupang 스크래핑 프롬프트

## 1. 목적
- Coupang 상품 정보 및 리뷰 등의 데이터를 수집하기 위한 요구사항 및 지시사항 정의

## 2. 수집 대상 데이터
- [ ] 상품명
- [ ] 가격
- [ ] 평점 및 리뷰 수
- [ ] 고객 리뷰 (제목, 내용, 별점)

## 3. 스크래핑 방식 및 주의사항
- 스크래핑 대상 URL:

 https://www.coupang.com/np/categories/305798?page=1

- 1) HTTP 요청정보: Request URL

Request URL
https://www.coupang.com/np/categories/305798?traceId=mqp7ucta&page=1
Request Method
GET
Status Code
200 OK
Remote Address
121.254.136.233:443
Referrer Policy
strict-origin-when-cross-origin

- 2) HTTP 헤더정보:

priority
u=0, i
referer
https://www.coupang.com/?src=1042016&spec=10304903&addtag=900&ctag=HOME&lptag=%EC%BF%A0%ED%8C%A1&itime=20260622215248&pageType=HOME&pageValue=HOME&wPcid=17783699071059948981675&wRef=www.google.com&wTime=20260622215248&redirect=landing&gclid=CjwKCAjwuuPRBhAnEiwA2Ji8ejfuBn4iCmMjcWRdMuwi0naR54AgCK9mHQA-48LlCUZbjb8j2UFFgRoCx5cQAvD_BwE&mcid=d2d84222538b435cb88214823028e2fa&campaignid=8704277940&adgroupid=86483039646&network=g
sec-ch-ua
"Google Chrome";v="149", "Chromium";v="149", "Not)A;Brand";v="24"
sec-ch-ua-mobile
?0
sec-ch-ua-platform
"Windows"
sec-fetch-dest
document
sec-fetch-mode
navigate
sec-fetch-site
same-origin
sec-fetch-user
?1
upgrade-insecure-requests
1
user-agent
Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/149.0.0.0 Safari/537.36

HEADERS = {
    "cookie": "
PCID=17783699071059948981675; MARKETID=17783699071059948981675; x-coupang-target-market=KR; x-coupang-accept-language=ko-KR; sid=efc0d5eb3b2249f68435b057b16d19ce69dcc91c; trac_src=1042016; trac_spec=10304903; trac_addtag=900; trac_ctag=HOME; trac_lptag=%EC%BF%A0%ED%8C%A1; trac_itime=20260627133837; bm_ss=ab8e18ef4e; bm_so=3357518B916B5B3B004FC03D50AA03F7FECF45E64C85E115B2143858E3F425E3~; bm_mi=1AFD132A0194E0CB836DC34BDD3A4F1D~~1; bm_lso=3357518B916B5B3B004FC03D50AA03F7FECF45E64C85E115B2143858E3F425E3~~1782535119569; bm_sz=3327338B26479C5B7ECEB5D6AAF0B4CC~~4534596~3293489; x-cp-s=; _abck=2E0430660BD46EF924CB45ED75DE9830~0~~-1~-1~1782538718~AAQAAAAF%2f%2f%2f%2f%2f1DnQhNV+3Hq3zXl2P44Mjqk%2fFHCHtXnKnbune6K4Lyn92WS%2fPbxuptVeGytVTXAvimOffFUjux1RHmAZ%2f4yNEwYm2AYbgjPoUVl~-1; ak_bmsc=6962E486D0D1602D285CFCF2C25C54F7~000000000000000000000000000000~; bm_sv=43C9FB71E37E2414F6C9D088ACD6120B~~1; bm_s=",
    "sec-ch-ua": '"Google Chrome";v="149", ... ',
    # ... 기존 내용 유지 ...
}


- 3) Payload 정보:

traceId=mqp7ucta&page=1


- 4) 응답 예시 (Response 일부 복사):

  {
                "@context": "https://schema.org",
                "@type": "BreadcrumbList",
                "itemListElement": [
                    {
                        "@type": "ListItem",
                        "position": 1,
                        "name": "쿠팡 홈",
                        "item": "https://www.coupang.com"
                    },
                    [
                        {
                            "@type": "ListItem",
                            "position": 2,
                            "name": "헬스/건강식품",
                            "item": "https://www.coupang.com/np/categories/305798"
                        }
                    ]
                ]
            }</script>
        <script type="application/ld+json">
            {
                "@context": "https://schema.org",
                "@type": "ItemList",
                "itemListElement": [
                    {
                        "@type": "ListItem",
                        "position": 1,
                        "item": {
                            "@type": "Product",
                            "name": "[정품] 자보티카바 정 Jaboticaba, 1개, 120정",
                            "image": [
                                "//thumbnail.coupangcdn.com/thumbnails/remote/230x230ex/image/vendor_inventory/ed1e/1d156690162871d130eb0a1f012f0f336eb997652c84c353c719139248a0.jpg"
                            ],
                            "offers": {
                                "@type": "Offer",
                                "price": 10900,
                                "priceCurrency": "KRW"
                            },
                            "aggregateRating": {
                                "@type": "AggregateRating",
                                "ratingValue": 5,
                                "reviewCount": 3396
                            },
                            "url": "https://www.coupang.com/vp/products/9360717888?itemId=27774354719&vendorItemId=94764769887"
                        }
                    },

- 5) 저장 요구사항: 한 페이지가 성공적으로 수집되는지 확인하고 csv 파일로 저장, 이후 전체 데이터를 수집하여 sqlitedb로 저장할 것.

## 4. 추가 주의사항
- 쿠팡은 봇 방지(Anti-Bot) 시스템이 강력하므로 `scrapling`, `Playwright` 또는 적절한 브라우저 위장(Stealth) 기술 사용
- 서버에 과도한 부하를 주지 않도록 딜레이(Delay) 설정
