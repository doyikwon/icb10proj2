"""
올리브영 카테고리 페이지에서 헤드풀(headless=False) 모드와 메인 페이지 세션 경유 방식을 결합하여 데이터를 안정적으로 수집하는 스크립트입니다.
"""
...
async def scrape_page():
    ...
    async with async_playwright() as p:
        print("헤드풀(headless=False) 모드로 브라우저를 기동합니다...")
        ...
        # 1. 메인 페이지 먼저 방문하여 세션 쿠키 발급 유도
        print(f"1. 메인 페이지 방문 중: {main_url}")
        await page.goto(main_url, wait_until="domcontentloaded")
        await page.wait_for_timeout(3000)
        
        # 2. 메인 페이지 상태에서 카테고리 페이지로 접근
        print(f"2. 카테고리 페이지 방문 중: {target_url}")
        await page.goto(target_url, wait_until="domcontentloaded")
        
        print("페이지 로딩 대기 및 스크롤 다운 수행 중...")
        ...
        if collected_data:
            ...
            print("수집된 상품 데이터가 없습니다. HTML 덤프 파일을 열어 태그 구조를 다시 확인해 주세요.")
